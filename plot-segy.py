...
# Reading Information of Seismic Data
...
def outputSeisSVG(myFile, nt, ns, t0, dt, seis, x0, y0, Width, Height, scale, kf):
    ...
    f.write("<g font-family='Verdana' font-size='16' fill='blue' stroke='blue' stroke-width='1'> \n")
    nx = math.floor(nt / 20)
    dx = 20 * nW / nt
    for k in range(nx + 1):
        xx = x0 + k * dx
    ...
    vmax = seis.max()
    vmin = seis.min()
    print("    Value Range: ( %.6f 0 +%.6f ) " % (vmin, vmax))
...
def plotSeismicMap(title, nPoint, nSample, t0, dt, seis, w, h, scale, kf):
    ...
    if(kf<1):
        ...
        # plot.contourf(xp1,yp1,seis,cmap=cmap)
    plt.grid(True)
    plt.show()
    return 0
...
if __name__ == '__main__':
    ...
    mySeisFile = "./input/yr99jd85-0616z.segy"
    myPlot = "./output/SEIS.html"
    ...
    plotSeismicMap(myTitle,nTrace,nSample,t0,dt,seis,Width,Height,scale,0)
    plotSeismicMap(myTitle,nTrace,nSample,t0,dt,seis,Width,Height,scale,1)
